$(document).ready(function(){
    ...
    $('.btn-exit').on('click', function(){
    	swal({
		 	...
		 	text: "Tu sesion será cerrada y seras dirigido fuera del sistema",
		  	type: 'Atención',
...
function checkRut(rut_clt){
	// Despejar Puntos
	var valor = rut_clt.value.replace('.','');
	// Despejar Guión
	valor = valor.replace('-','');
	...
	// Formatear RUN
	rut_clt.value = cuerpo + '-'+ dv
	
	// Si no cumple con el mínimo ej. (n.nnn.nnn)
	if(cuerpo.length < 7) { rut_clt.setCustomValidity("RUT Incompleto"); return false;}
